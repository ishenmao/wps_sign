...
import json
import sys
# wps ID列表，需要做邀请任务的账号ID，可以添加多个
id_list = []

# wps_sid，需要做稻壳签到任务的 sid
sid_list = []
...
def fool_print(cnt = 10):
    
    print('今日已成功邀请 %d 人' % cnt)
    print('今日已自动打卡')
    print('稻壳签到已完成')

def docer_checkin(sid: str):
    base_url = "https://zt.wps.cn/2019/docer_sign_ppt/api/base_info"
    s = requests.session()
    r = s.get(base_url, headers = {'sid': sid}).json()
    if r['data']['is_checkin_today']:
        print("今天已经签到过了")
        return
    # 7:00-14:00不需要答题
    WxCode = '011Jq1jn1G3NMo0oisjn1i3ojn1Jq1jQ'
    url = "https://zt.wps.cn/2019/docer_sign_ppt/api/checkin"
    r = s.post(url, headers={'sid': sid, 'Wx-Code': WxCode}, data={'is_question':0})
    #res = r.json()
    print(r.text)
...
def invite(user_id):
    ...
    s = requests.session()
    for sid in others_sid:
        r = s.post(invite_url, headers = {'sid': sid}, data = {'invite_userid': user_id})
        # debug
        print(r.text)
        count['total'] += 1
        try:
            ...
            if rjson['result'] == 'ok':
                ...
                count['fail'].append({'sid':sid[:6]+'***'+sid[-4:], 'msg':rjson['msg']})
        except json.JSONDecodeError:
            count['fail'].append({'sid':sid[:6]+'***'+sid[-4:], 'msg':'unknown error'})  
        if count["succ"] >= 10 or sid is others_sid[-1]:
            break
...
def main():
    ...
    for uid in id_list:
        ...
        push_txt += "\nWPS签到的邀请推送"
        push_txt += "\n尝试为【ID：%d】邀请 %d 位用户，其中：" % (uid, count['total'])
...
def get_args():
    ...
    for item_sid in sys.argv[5].split(','):
